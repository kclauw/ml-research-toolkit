...
def download_runs(
    ...
    save_csv: bool = True,
    download_artifacts: bool = True,  # New argument
) -> List[str]:
    """
    Download W&B runs for a given project and save their configurations, selected metrics, and optionally artifacts.
    ...
    Args:
        ...
        download_artifacts (bool, optional): Whether to download run artifacts like weights/checkpoints. Defaults to True.
    ...
    for run in runs:
        ...
        # Download artifacts (weights/checkpoints)
        if download_artifacts:
            for artifact in run.logged_artifacts():
                artifact_dir = os.path.join(output_folder, artifact.name)
